...
`connect error: <n>`, or an OTA `image valid, signature bad` that never reaches HTTP. With Syslog
configured, all of those reach the collector too — including `BOOT reset_reason=`, which
