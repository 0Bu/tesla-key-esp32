...
> Invoke this workflow canonically as `$device-diag`.
...
authorizes the live boundary and identifies the target host; a generic review or diagnosis request
without that target approval does not authorize even a passive HTTP call.

The default diagnostic request does **not** authorize `?verbose=1`, `?verbose=0`, `?clear=1`, a
...
`handle_status` in [`main/http_status.cpp`](../../../main/http_status.cpp) returns exactly the JSON
the web UI itself lives on — `app.js` polls this same route every 4 s. Top-level keys: `vin`, `ip`, `version`, `paired` (=
...
```bash
curl -s 'http://<host>/diag'              # default read-only dump
```

Query params (parsed in `handle_diag`) are deliberately **not part of the default read-only flow**:
`?verbose=1`/`?verbose=0` mutate raw-RX capture state and `?clear=1` clears the ring. If the user
explicitly authorizes that diagnostic mutation, use only the agreed parameter and restore verbose
mode afterwards. The response header `X-Diag-Verbose` echoes the current mode.
...
```bash
# Host serial monitor (no local idf.py) — only after explicit hardware approval; exit Ctrl-A then K.
screen /dev/cu.usbmodemXXXX 115200
```

Use serial only as an explicitly approved fallback whenever the board is off the LAN (some USB
serial adapters toggle reset/control lines when opened), or when no Syslog collector is configured and you need the `BOOT`/`HEAP` lines, a
classified `BLE connect gave up …`, or an OTA `image valid, signature bad` that never reaches HTTP.
Raw `connect error: <n>`/GATT attempt codes are DEBUG-only and require a diagnostic build compiled
with maximum DEBUG; the normal maximum-INFO build deliberately omits them. With Syslog configured,
the production-level lines reach the collector too — including `BOOT reset_reason=`, which
`main.cpp` emits as the first line past `syslog_start()` precisely so a restart can be explained
after the fact.
...
|---|---|---|
| **Every BLE connect times out.** `/diag`/syslog shows the classified `BLE connect gave up after …` first occurrence/heartbeat; `/status` has `ble.connected:false`, `ble.connect_fail` climbing, often `ble.car_connectable:false`. A maximum-DEBUG diagnostic build may additionally show raw `connect error: 13` (`BLE_HS_ETIMEOUT`). | A **second board** holding the car's single BLE link (Tesla allows ~3 simultaneous, and a proxy holds it continuously), **or** signal-limited RX (weak advert RSSI). `ensure_connected_` classifies only adverts from the current attempt: `car advertised but the connect did not complete` versus `car not in BLE range` (nothing found) or `non-connectable` (at the BLE limit). Automatic unchanged conditions log first occurrence plus a monotonic hourly heartbeat; `/status` continues counting between those lines, so a low production-log count does **not** mean attempts stopped ([`main/logic/connect_outcome.hpp`](../../../main/logic/connect_outcome.hpp), raw DEBUG callbacks in [`main/ble_client.cpp`](../../../main/ble_client.cpp)). | Run **one board per car** — power off the other holder, then retry. If truly alone, treat as an antenna/range problem (physical), not a flash. |
| **`/status` reports `last_reboot:"heap:<n>"`** — the device is healthy NOW, with a low uptime nobody triggered. `n` = consecutive watchdog restarts; ≥2 means restarting is not fixing it, and at 5 it stops restarting. | The **heap-exhaustion watchdog** restarted us: `largest_block` stayed under 4 KB for 5 unbroken minutes, i.e. the heap was gone and not coming back ([`main/logic/heap_watchdog.hpp`](../../../main/logic/heap_watchdog.hpp), sampled in `vehicle_telemetry.cpp` `loop_task_fn_`). The restart is the *symptom being handled*, not the fault — something leaked. The field is set once per boot and absent on every ordinary one. | Find the leak, don't just restart. Pull the `HEAP free=… largest_block=…` trend from **syslog** (the in-RAM `/diag` ring does not survive the reboot) and look for what ran before the decline — the known one was an unbounded send backlog in the WebSocket status push that used to serve the web UI (since replaced by polling). If it recurs, that is a real leak to fix, not a watchdog to loosen. |
