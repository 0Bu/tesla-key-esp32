...
> Invoke this workflow canonically as `$e2e-evcc`.
...
> **Live authorization boundary.** Do not run even the nominally read-only path merely because a
> build, flash, review, or diagnosis completed. First obtain explicit user authorization to contact
> the live evcc pod and board. `RUN_COMMANDS=1`, `ALLOW_CHARGE_TOGGLE=1`, and
> `RUN_ALL_COMMANDS=1` are three progressively broader mutation scopes; approval for the read path
> or one command scope never authorizes another.

## How to run

After explicit live-test approval, start with the read path (this is the part that matters for evcc):

```bash
bash scripts/e2e_evcc.sh
```

The write path issues real signed-BLE commands to the car. Obtain a separate explicit command-test
approval and confirm the exact scope first (see below):
...
## Before running the write path — explicit, scope-specific approval
