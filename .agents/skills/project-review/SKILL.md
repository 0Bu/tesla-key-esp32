...
| Board identity | `main/board.{cpp,hpp}` | Runtime board detection for the ONE image per chip. The esp32s3 image serves the T-Dongle-S3, a bare ESP32-S3, and several W5500 PoE/Ethernet boards (AtomS3 Lite + ATOMIC PoE Base, Waveshare ESP32-S3-ETH, LilyGO T-ETH-Lite), the W5500 set probed by `main/logic/eth_board.hpp`'s candidate table. ONE cached detector because display and Ethernet OVERLAP ON A PIN — the panel's SPI clock is GPIO5, the PoE base's SCLK |
