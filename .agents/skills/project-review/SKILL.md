...
- **No HTTP auth / TLS by design** (evcc can't send credentials) — trusted LAN only. Mutating
  browser requests add a narrower same-Origin/`Sec-Fetch-Site` gate while headerless evcc/curl
  remains allowed; do not mistake that CSRF mitigation for LAN-client authentication. Document
