...
| BLE orchestration seam | `main/logic/command_runner.hpp` + `main/logic/ble_dispatcher.hpp` + `main/logic/rx_framing.hpp` + `main/logic/session_state.hpp` + `main/logic/command_result.hpp` + `main/logic/key_rotation.hpp` + `main/logic/ble_chunk.hpp` + `main/logic/ble_deferred_event.hpp` | The pure-logic BLE orchestration seam: command scheduling, timeout arbitration, session-state caching, RX deframing, anti-replay, and deferred BLE event handling without IDF/FreeRTOS dependencies |
