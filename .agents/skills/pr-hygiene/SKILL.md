...
- A full or partial LAN IP address standing in for a real device (`192.168.1.163`, or the
  shorthand `.163`/`.194` this project's own PR history used for "the board on the bench") —
  not a documentation placeholder (`192.0.2.x`, `<ESP32-IP>`, generic `192.168.1.42`-style
  examples) and not a universal address (`0.0.0.0`, `127.0.0.1`, a loopback proxy URL).
- A real device MAC address — the vehicle's own, a nearby BLE peer's, or a network interface's
  (Ethernet/W5500, WiFi AP/mesh node) — even partially masked (`28:70:…:f9:9a`). A generic
  placeholder (`aa:bb:cc:dd:ee:ff`) is not a finding.
...
- A personal email address, phone number or physical address. A well-known convention is not a
  finding on its own — `git@github.com` in an SSH remote (`git@github.com:owner/repo`) is
  standard syntax, not a leak.
...
2. **Classify each piece of text** against the two finding categories above. Do not flag a
   generic/placeholder value, a universal address, a well-known syntax convention, or a quoted
   historical fact about what code/UI used to output.
