...
> Invoke this workflow canonically as `$display-preview`.
...
| [`$add-logic-test`](../add-logic-test/SKILL.md) | Adds the pure-logic units + `CHECK` cases (the presenter *decisions* are host-tested there). |
...
**Then look at the output.** Inspect/attach the PNGs (`tools/display_states.png`, etc.) and
...
- To see the rendered frames on an actual panel instead, build + flash a T-Dongle-S3 with the
  [`$flash-esp32`](../flash-esp32/SKILL.md) skill (each BOOT tap rotates 90° through the
  four orientations).
