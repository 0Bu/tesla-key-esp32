...
compact, canonical instruction set for this repository and is loaded by Claude Code as project
memory. Read deeper references only when the task touches them:
...
skills use `$skill-name` and canonical paths under `.claude/skills/`; `/skill-name` is the Claude
Code invocation spelling and stays accepted in PR gate records. Hook policy lives in
`tools/agent-hooks/`.
...
Lifecycle and PR policy is implemented in [`tools/agent-hooks/`](../tools/agent-hooks/) and wired
for this project by [`settings.json`](settings.json). Read-only specialist reviewers live in
[`agents/`](agents/). Hooks are lexical defense in depth: they do not replace sandboxing, explicit
authorization, branch protection, protected environments or human review.
