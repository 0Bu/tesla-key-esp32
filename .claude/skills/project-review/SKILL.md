...
supports, plus esp32c5 via `prepare-tesla-ble-c5.sh` + the conditional
`main/idf_component.yml`). All five receive the repository-owned replay-rejection patch in
...
| Library | `managed_components/yoziru__tesla-ble/` / `third_party/tesla-ble/` | **fetched, regenerated — NEVER edit or commit** (pin in `main/idf_component.yml`); root CMake applies committed `patches/tesla-ble/` through `scripts/apply-tesla-ble-patches.sh` |
...
- **`multi-target-build-reviewer`** is the per-target build/config divergence lens (the five
  ...
  local tesla-ble target routing (`prepare-tesla-ble-c5.sh` + `main/idf_component.yml`) plus the
