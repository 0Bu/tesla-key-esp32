...
| Network transport | `main/net.{cpp,hpp}` + `logic/net_link.hpp` | The ONE transport seam: everything above it asks `tk::net_is_up()` / `net_kind()` / `net_active_netif()` and never touches `esp_wifi`. Owns the WiFi station, the endless-reconnect handler, the credential-rollback boot window (`logic/wifi_rollback.hpp`) and the gateway-ICMP ghost-link watchdog (`net_wd`); the transport identity `NetLink::{None,Wifi,Eth}` and the watchdog's decision (incl. the never-answered-ICMP baseline rule) are host-tested in `logic/net_link.hpp`. Also the OPTIONAL W5500 SPI Ethernet backend (`CONFIG_TESLA_ETH_ENABLED`, esp32s3 only): VERSIONR probe, POLLING mode (the PoE base routes no INT/RST), bounded lease wait with WiFi fallback, and — on a lease — WiFi is never started at all |
...
| Heap trend | `main/heap_trend.{cpp,hpp}` + `logic/heap_history.hpp` | the board's own 24-hour free/largest-block ring (`GET /heap`), fed from the SAME two samples `loop_task` hands the heap watchdog, so the chart a human reads and the threshold the firmware acts on cannot disagree. Fixed `.bss` (~1.2 KB), **never heap** — a diagnostic must not compete for the block it exists to measure |
...
- **`multi-target-build-reviewer`** is the per-target build/config divergence lens (the four
  ...
  (`0x1000`/`0x0`), the image-suffix map (`ci-build-all.sh`/`build-pages.sh`/
...
- Don't propose a hand-edit to `managed_components/`. Prefer a wrapper guard or pin bump; when
  the defect is inside library dispatch and no upstream release contains the fix, commit a
  minimal patch under `patches/tesla-ble/` and apply it deterministically to every target.
