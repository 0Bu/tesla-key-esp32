...
> boundaries in [`CLAUDE.md`](../../CLAUDE.md).
...
1. **Build the intended model from the docs first.**
   ...
   agree with the code without copying those long technical catalogs back into `CLAUDE.md`.
...
- **`$add-logic-test`** scaffolds a new pure-logic unit in `main/logic/` + its `CHECK` cases in
  ...
  `tools/agent-hooks/agent_hook.py` wired by `.claude/settings.json`, the
...
- **`$feature-docs`** keeps `docs/FEATURES.md` in sync when a platform feature lands or changes.
  ...
  that the policy-path set covers `.claude/`,
...
- **`agent-config-reviewer`** audits project agent configuration, skills, hooks, and
  subagents—not firmware logic. Confirm its boundary still points firmware-correctness work back
  at this skill and its inventory matches `.claude/`, and
  `tools/agent-hooks/`.
...
re-read the doc that documents it. Project hooks in `.claude/settings.json` must delegate to the
