...
| Pure logic (host-tested) | `main/logic/*.hpp` (`vin`, `units`, `link_state`, `target`, `mcp`, `command_result`, `ui_state`, `display_model`, `soc_gradient`, `led_status`, `syslog_policy`, `ws_policy`, `active_window`, `ha_templates`, `http_body`) + `test/test_logic.cpp` | **IDF-free** logic the device and the host test share: VIN validation, imperial→metric units, the `link_state()` four-state machine + its `/status`/MQTT strings, per-target platform/OTA-suffix map, the MCP tool/arg-spec registry + JSON-RPC routing/version negotiation, the shared command result-string mapping, and the on-device display presenter + status-LED ladder (both off the shared `ui_state` snapshot + `soc_gradient` ramp). Host-built + run by `scripts/run-mock-tests.sh` (CI `logic-test` gate) — the real local verification loop; the same script also parity-checks `tools/display_sim.py` against the C++ display presenter (`scripts/check-display-sim-parity.sh`) |
