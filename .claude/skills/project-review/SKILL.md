...
8. **Record the pass in the PR so the gates clear (no file marker).** A clean review (no
   *blocking* findings) must **tick the PR's `/project-review` checkbox and stamp it with the
   ...
   stamped sha still matches the PR head. Bash uses the canonical
   `gh pr merge <number-or-local-GitHub-URL> --match-head-commit <full-PR-head> ...` form with the
   selector and literal race-closing precondition first; foreign URLs, implicit/branch/flag-first
   targets, `--admin`, auto-merge and missing/wrong/duplicate head preconditions fail closed. Both
   Bash and structured MCP merge require exact equality between PR head and local audited HEAD;
   MCP additionally requires exact github.com `owner/repo` plus `expected_head_sha`:
