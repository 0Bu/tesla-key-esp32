---
name: feature-docs
description: Keep docs/FEATURES.md (this firmware's technical-feature catalog) in sync when a platform feature lands or changes — including project agent policy under .claude/, .github/PULL_REQUEST_TEMPLATE.md, tools/agent-hooks/, and tools/agent-config/. Use after implementing or changing a technical feature, before opening the PR. Required before merging a PR whose diff reaches those agent-policy paths, main/, test/, sdkconfig.defaults, partitions.csv, the shipped Pages runtime, release-relevance.sh, or the build/signed-preview/preview-cleanup/PR-policy/bench-acceptance workflows.
---
...
PR's diff reaches `main/`, `test/`, `sdkconfig.defaults*`, `partitions.csv`, project
agent policy (`.claude/`, `.github/PULL_REQUEST_TEMPLATE.md`,
...
2. **Decide whether a technical feature moved.** It did if the change:
   ...
   - changes project instructions, skills, reviewer/config registration, lifecycle gates, or
     their validators under `.claude/`,
...
5. **Check the cross-references.** If the feature is also described in
   `CLAUDE.md`, `docs/ARCHITECTURE.md`, `docs/SECURITY.md` or `README.md`, make sure they
