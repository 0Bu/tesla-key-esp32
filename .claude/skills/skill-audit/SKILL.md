...
The `.claude/skills/*/SKILL.md` files and read-only reviewers in `.claude/agents/*.md` are
...
1. **Enumerate—discover, do not hardcode.** Read every `.claude/skills/*/SKILL.md` and every
   `.claude/agents/*.md`. Inventory `CLAUDE.md`, `.claude/settings.json`, `tools/agent-hooks/`,
...
3. **Verify claims against the tree.** Use plain file reads/search (`rg` preferred) and
...
- **`$add-logic-test`** — scaffolds a `main/logic/` unit + `CHECK`s in `test/test_logic.cpp`.
  ...
  `tools/agent-hooks/agent_hook.py` wired by `.claude/settings.json`,
...
- **`agent-config-reviewer`** — audits project agent configuration, not firmware logic. Confirm
  its read-only boundary and inventory of `.claude/`, and
  `tools/agent-hooks/`.
...
- **Where:** `.claude/…` or `.claude/…:line` → ground truth: `path:line`
