...
gate_checkbox_status() {
  ...
  # Match a markdown task-list item ("- [ ]" / "- [x]", any bullet) mentioning <key>. NOT anchored
  # to line start: for a `gh pr create --body '...'` call the whole command is one line and the box
  # sits mid-line — the bullet+checkbox sequence is distinctive enough to find it there too.
  line="$(printf '%s\n' "$content" \
      | grep -iE '[-*][[:space:]]+\[[ xX]\]' \
      | grep -F -- "$key" | head -n1)"
...
#   Reads an EXISTING PR for the current branch (selector: a PR number/branch/url, or empty for
#   the current branch). On success prints the head sha on line 1, then the PR body. Returns 1 if
#   the PR could not be read (no PR, or no gh AND no usable token).
gate_fetch_pr() {
  local sel="$1" json tok slug num owner branch
  if command -v gh >/dev/null 2>&1; then
    # shellcheck disable=SC2086
    json="$(gh pr view $sel --repo "$(gh repo view --json nameWithOwner -q .nameWithOwner 2>/dev/null)" \
            --json body,headRefOid 2>/dev/null)" || json=""
    if [ -z "$json" ]; then json="$(gh pr view $sel --json body,headRefOid 2>/dev/null)" || json=""; fi
    if [ -n "$json" ]; then
  ...
  { [ -n "$tok" ] && command -v curl >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; } || return 1
  slug="${GATE_REPO_SLUG:-$(gh repo view --json nameWithOwner -q .nameWithOwner 2>/dev/null)}"
  [ -n "$slug" ] || return 1
  if printf '%s' "$sel" | grep -qE '^[0-9]+$'; then
    num="$sel"
  else
    branch="$(gate_branch)"; owner="${slug%%/*}"
    num="$(curl -fsSL -H "Authorization: Bearer $tok" -H "Accept: application/vnd.github+json" \
        "https://api.github.com/repos/$slug/pulls?head=$owner:$branch&state=open" 2>/dev/null \
        | jq -r '.[0].number // empty')"
  fi
  [ -n "$num" ] || return 1
  json="$(curl -fsSL -H "Authorization: Bearer $tok" -H "Accept: application/vnd.github+json" \
      "https://api.github.com/repos/$slug/pulls/$num" 2>/dev/null)" || return 1
  [ -n "$json" ] || return 1
  printf '%s\n' "$(printf '%s' "$json" | jq -r '.head.sha // ""')"
  printf '%s'   "$(printf '%s' "$json" | jq -r '.body // ""')"
  return 0
}
