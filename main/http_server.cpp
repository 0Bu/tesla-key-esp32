...
".loghd{display:flex;align-items:center;justify-content:space-between;margin-bottom:1.1rem}.loghd h2{margin:0}"
".icon{font-size:1.05rem;line-height:1;padding:.4rem .55rem;border:1px solid var(--border);border-radius:10px;background:var(--card);cursor:pointer}.icon:active{transform:translateY(1px)}"
"</style>"
"<main>"
...
"</ol></section>"
"<section class=card><div class=loghd><h2>LOGS</h2><button class=icon title='Download diagnostics' onclick=dgDownload()>&#128030;</button></div>"
"<div id=log>Ready.</div></section>"
"</main>"
"<script>"
"let V='';"
"function log(t){document.getElementById('log').textContent=t}"
"async function dgDownload(){try{let r=await fetch('/diag');let t=await r.text();"
"let b=new Blob([t],{type:'text/plain'});let a=document.createElement('a');"
"a.href=URL.createObjectURL(b);a.download='diag.txt';a.click()}catch(e){log('diagnostics download failed')}}"
