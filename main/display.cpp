...
// The network seam (net.hpp). The display task is created (display_start) BEFORE the link
// comes up and at higher priority than app_main, so it MUST NOT read esp_wifi itself: the
// station record has transiently-null fields while associating and a concurrent read there
// faults (LoadProhibited, EXCVADDR=0x1). tk::net_wifi_signal() owns that gate. No link →
// the centre network-search animation is shown instead (the correct state anyway).
...
// net=true → the word "WiFi"/"LAN" (scale 2); else the Bluetooth glyph (scale 3), centered.
static void draw_portrait_search(int frame, bool net, bool lan = false) {
    if (net) {
        const char* w = lan ? "LAN" : "WiFi";
        draw_text((W - text_w(w, 2)) / 2, 52, w, C_INK, 2);
    } else {
        blit_rows((W - DISPLAY_BT_W * 3) / 2, 46, DISPLAY_BT_ROWS,
...
static bool draw_landscape(const tk::display::Model& m, const tk::UiSnapshot& s, int tick) {
    // header: the network cluster; hidden while the network is the active search hero.
    // Ethernet gets a static "LAN" label rather than bars — a wire has no signal level, and a
    // fixed 4/4 would be a fabricated reading rather than a missing one.
    if (m.show_lan) draw_text(4, 4, "LAN", C_INK, 2);
    // WiFi bars + SSID (scale 2). An over-long SSID scrolls (clipped marquee) at the
    // presenter-computed offset.
    if (m.show_wifi) {
    ...
    switch (m.hero) {
        case tk::display::Hero::NetSearch:
            // The label names what THIS board is waiting for: a PoE device that displays
            // "WiFi …" while it waits for DHCP over a wire is simply lying.
            if (m.search_is_lan) draw_searching_text(tick, "LAN", 2, 20, 42);
            else                 draw_searching_text(tick, "WiFi", 2, 14, 42);
            return true;
...
static bool draw_portrait(const tk::display::Model& m, const tk::UiSnapshot& s, int tick) {
    // header row 1: network (left) | BT glyph + BLE bars (right); row 2: SSID; divider at y31.
    if (m.show_lan) draw_text(4, 4, "LAN", C_INK, 2);     // a wire has no signal level
    if (m.show_wifi) {
        signal_bars(4, 4, m.wifi_bars, C_BAR_ON);
    ...
    switch (m.hero) {
        case tk::display::Hero::NetSearch:
            draw_portrait_search(tick, /*net*/true, /*lan*/m.search_is_lan);            return true;
        case tk::display::Hero::Pairing:    draw_portrait_pairing(tick);                return true;
        case tk::display::Hero::BleSearch:  draw_portrait_search(tick, /*net*/false);   return true;
...
static bool render_frame(VehicleController& v, int tick, bool paired) {
    ...
    // sample; the network fields come from the transport seam, which is what makes the
    // esp_wifi read safe (see the note at the top of this file).
    tk::UiSnapshot s = v.ui_snapshot();
    s.paired      = paired;
    s.net         = tk::net_kind();
    s.eth_present = tk::net_eth_present();
    // RSSI + SSID exist only on a radio; on Ethernet both stay at their defaults and the
    // presenter draws the "LAN" label instead of fabricating 4/4 bars.
    //
    // A WiFi lease whose station record cannot be read RIGHT NOW (the brief window while the
    // STA is re-associating) is downgraded to "searching" rather than drawn: leaving it as
    // Wifi would render rssi 0 dBm — which rssi_bars() reads as a perfect 4/4 — so a link in
    // the middle of dropping would show its strongest possible signal. That was the old
    // behaviour of the `wifi_is_connected() && ap_info_ok` conjunction and it must survive
    // the split into two calls.
    if (s.net == tk::NetLink::Wifi && !tk::net_wifi_signal(&s.wifi_rssi, s.ssid))
        s.net = tk::NetLink::None;
...
void display_start(VehicleController& vehicle) {
    ...
    if (!tk::board_is_t_dongle_s3()) {
        ESP_LOGI(TAG, "no T-Dongle-S3 detected — display disabled");
        return;
    }
#endif
