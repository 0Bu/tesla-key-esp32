...
| Safe host provisioning | `provision.py`, setup `POST /save`, LAN `POST /set_wifi` | A generated partial NVS image replacing the entire partition and silently deleting the vehicle private key/sessions. Serial partition writes are retired; passwords use a prompt/stdin/0600 file, setup is atomic, and reachable-device WiFi changes retain the one-shot rollback. |
