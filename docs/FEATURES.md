...
| Bug-report redaction | `logic/redact.hpp`, `GET /status?redact=1`, `GET /diag?redact=1` | A diagnostic snapshot carrying the reporter's VIN, SSID, IP, BLE MACs, broker or syslog host into a public issue. Fails closed; substitutes the value and keeps the key. |
...
| 24-hour heap trend (`GET /heap`) | `logic/heap_history.hpp`, `main/heap_trend.cpp` | A spot value that cannot show DRIFT. A leak is a slope; fragmentation is the two lines separating. Fixed `.bss` ring (~1.2 KB), never heap — a diagnostic must not compete for the resource it measures. |
