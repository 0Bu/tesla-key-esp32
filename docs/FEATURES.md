...
| Browser installer port release | `docs/index.html`, `docs/serial-port-release.mjs`, `test/serial_port_release.test.mjs`, `scripts/build-pages.sh` | A previously granted serial port staying marked "paired" with no user-facing way to revoke the site's permission — or a release action interrupting an active flash. The browser's native chooser identifies the target; `SerialPort.forget()` runs only while the port is closed, and the control stays hidden when the API is unavailable. |
