...
- **Sleep state evaluation vs wake advancement (R1)**: The raw `vehicleSleepStatus` from VCSEC determines the reported sleep state (`vcsec_sleep_state_`). A VCSEC `AWAKE`, or a `VehicleStatus` carrying `closureStatuses`, that arrives while a command waits for its wake confirms that wake for the command (`CommandRequest::wake_confirmed`). Unlike PR #322 where `wake_confirmed` was sticky across retries, on retry or step timeout `wake_confirmed` is reset to false; if the vehicle is still reported asleep (`is_asleep`), `tick()` re-evaluates sleep and emits `TxAction::SendWake`.
- **VCSEC operation status handling (F3)**: Upstream `vehicle-command` treats VCSEC command status with `operationStatus` `OPERATIONSTATUS_WAIT` as an in-progress indicator, `OPERATIONSTATUS_OK` as success, and others as errors (`pkg/vehicle/vcsec.go`). The firmware evaluates `operationStatus`: `OPERATIONSTATUS_WAIT` keeps waiting for completion, `OPERATIONSTATUS_OK` completes the command successfully, and error statuses abort with an error.
- **Dispatcher-bypass protection and telemetry handling on unauthenticated frames (F2)**: In `vehicle-command` (`dispatcher.go:287-292`), unmatched responses are dropped before callbacks are invoked. In this firmware: (1) in `main/vehicle_telemetry.cpp`, incoming `SessionInfo` frames carrying a mismatched `request_uuid` are dropped before tag verification; SessionInfo HMAC errors or signed message faults only fail an in-flight command if the command is actively awaiting authentication (`WaitingVcsecAuth` or `WaitingInfoAuth`) for that specific domain; orphan or mismatched frames are dropped fail-closed without aborting unrelated active commands; (2) VCSEC `VehicleStatus` frames update `vcsec_sleep_state_`, the ASLEEP debounce clock, and invoke `vehicle_status_callback_` (satisfying `get_vehicle_status()` generation polling) before `handle_response()` routes the frame against the command queue, preserving passive sleep tracking and status telemetry even when `VehicleStatus` arrives unrequested or as a broadcast.
...
The native `tk::RxFramer` reassembly implementation covers the five critical framing boundary conditions identified during v5.1.4 analysis:
- **Case 2 (Zero-length rejection)**: Frames with decoded length 0 are rejected immediately as `CorruptLength`, preventing infinite or zero-byte parsing loops.
- **Case 3 (Incomplete chunk stream)**: When chunks cease arriving before a complete frame is reassembled, the buffer times out (`InterChunkTimeout`) and is discarded, restoring clean framing for subsequent messages.
- **Case 4 (Oversized frame rejection)**: Frames specifying a length header exceeding `kMaxFrameLength` (2048 B) are rejected fail-closed as `TooLarge`.
- **Case 13 (Multi-frame concatenation / back-to-back notifications)**: Multiple complete frames or back-to-back chunk sequences are sliced and processed sequentially without buffer corruption.
- **Case 14 / 14a (Inter-chunk timeout trade-off)**: While `vehicle-command` specifies a 1000 ms timeout, `tk::CommandRunner` configures an inter-chunk timeout of 3000 ms (`kDefaultRxInterChunkTimeoutMs`). This trade-off tolerates typical ESP32 BLE radio scheduling delays and latency spikes on noisy links, while accepting that a genuinely torn frame will retain the RX buffer for up to 3000 ms before reset.
