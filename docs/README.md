...
**charging bolt** while charging (hidden at 100 %) and an **ASLEEP** (dimmed) state. When
there is no usable link to the car (unreachable, or no cached SoC yet) the battery is replaced
by an animated **BLE "SEARCHING"** indicator — a green highlight sweeps across light-green
bars — and the header's BLE symbol is hidden. All from cache-only state, so it never wakes the
car and does not depend on MQTT. Offline pixel-exact validation: `python3 tools/display_sim.py
states` (every state) and `python3 tools/display_sim.py search` (the searching animation).
