...
# These tests compile against the exact cJSON sources of the espressif/cjson registry release the
# target lockfiles pin, re-hashed against the locked component hash. Run their sanitizer modes once
# before spending time on eight target builds (four primary + four isolated reproducibility rebuilds).
