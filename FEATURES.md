...
| Inline Web Serial installer + monitor | `docs/index.html`, `docs/web-installer.mjs`, `test/web_installer.test.mjs` | A generic browser dialog hiding chip mismatches, erase scope and flash progress. The page probes all four supported ESP targets, selects their sparse manifest parts, makes the NVS-preserving boundary explicit, and exposes reset plus the real 115200-baud boot log. |
| Browser installer port release | `docs/index.html`, `docs/serial-port-release.mjs`, `test/serial_port_release.test.mjs`, `scripts/build-pages.sh` | A previously granted serial port staying marked "paired" with no user-facing way to revoke the site's permission — or a release action interrupting an active flash. `Serial.getPorts()` keeps the control hidden unless permission already exists; `SerialPort.forget()` runs only while the port is closed. |
