#!/usr/bin/env bash
# Compile the MQTT build/print/publish gate against the exact cJSON source the firmware links.
set -euo pipefail
...
if [ ! -f "$cjson_cache/.complete" ]; then
    ...
    if [ "$(git config -f "$cjson_cache/gitmodules" --get submodule.cjson/cJSON.url)" != "$cjson_upstream" ]; then
        echo "MQTT JSON gate: locked cjson component no longer vendors $cjson_upstream" >&2
        exit 1
    fi
...
if ! [[ "$cjson_submodule_commit" =~ ^[0-9a-f]{40}$ ]] ||
   ...
   [ "$(git -C "$cjson_cache/cJSON" rev-parse HEAD)" != "$cjson_submodule_commit" ]; then
    echo "MQTT JSON gate: cJSON checkout does not match locked component $cjson_commit" >&2
    exit 1
fi
...
if [ ! -f "$cjson_source" ] || [ ! -f "$cjson_header" ]; then
    echo "MQTT JSON gate: locked cJSON source/header not found under $cjson_dir" >&2
    exit 1
fi
