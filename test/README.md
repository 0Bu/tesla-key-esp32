...
| Heap-exhaustion watchdog (4 KB INTERNAL `largest_block` threshold, 5 min UNBROKEN hold, recovery resets the clock, OTA clears the run, 32-bit tick-wrap safety, 5-restart cap, `heap:<n>` breadcrumb round-trip + unparseable-input handling) | `logic/heap_watchdog.hpp` | `vehicle_telemetry.cpp` `loop_task_fn_`, `vehicle_ctrl.cpp` `init()` |
